...
export default function PlanApp() {
  ...
  // Keep URL in sync with the active tab's session
  useEffect(() => {
    const path = activeTab?.sessionId
      ? `/plan/${activeTab.sessionId}`
      : "/plan";
  ...
  const send = async () => {
    ...
    try {
      ...
      setMessages((prev) => [
        ...prev,
        { role: "assistant", content: accumulated || "Your itinerary has been planned!" },
      ]);
